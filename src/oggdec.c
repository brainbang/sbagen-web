...
// Tremor's ov_read has 4 args: (file, buffer, length, bitstream)
#define OV_READ(file, buf, len, sect) ov_read((file), (buf), (len), (sect))
#else
#include <vorbis/codec.h>
#include <vorbis/vorbisfile.h>
// Standard vorbisfile's ov_read has 7 args, we want 16-bit little-endian signed
#define OV_READ(file, buf, len, sect) ov_read((file), (buf), (len), 0, 2, 1, (sect))
#endif
...
looper_init() {
  ...
  if (0 != fseek(mix_in, 0, SEEK_END) || 0 > (datlen = ftell(mix_in))) {
    error("Can't determine length of OGG file: %s", strerror(errno));
  }

  data = ALLOC_ARR(datlen, char);
  data_end = data + datlen;
  if (0 != fseek(mix_in, 0, SEEK_SET) || 1 != fread(data, datlen, 1, mix_in)) {
  ...
  if (fade_cnt < datrate / 50) {
    fade_cnt = datrate / 50; // 20ms min fade
  }
...
#define CNT_TO_END(aa)                                                                      \
  (aa->mode == 1 ? aa->cnt_all + aa->cnt : aa->mode == 2 ? aa->cnt_all - fade_cnt + aa->cnt \
...
looper_sched() {
  ...
  while (1) {
    ...
    if (rv) {
      error("UNEXPECTED: Can't reseek the looping OGG stream: %d", rv);
    }
  }
