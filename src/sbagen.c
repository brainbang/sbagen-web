...
// Define NO_AUDIO if no audio output device is usable (SDL is used by default)
...
// SDL_AUDIO is always available now
...
help() {
  printf(
      "SBaGen - Sequenced Binaural Beat Generator, version " VERSION NL
      "Copyright (c) 1999-2011 Jim Peters, http://uazu.net/, all rights " NL
      "  reserved, released under the GNU GPL v2.  See file COPYING." NL NL
      "Usage: sbagen [options] seq-file ..." NL
      "       sbagen [options] -i tone-specs ..." NL
      "       sbagen [options] -p pre-programmed-sequence-specs ..." NL NL
...
#ifndef MAC_AUDIO
      NL
      "          -b bits   Select the number bits for output (8 or 16, default "
      "16)"
#endif

      NL
      "          -L time   Select the length of time (hh:mm or hh:mm:ss) to "
      "output" NL "                     for.  Default is to output forever." NL
      "          -S        Output from the first tone-set in the sequence "
      "(Start)," NL
      "                     instead of working in real-time.  Equivalent to "
      "'-q 1'." NL
      "          -E        Output until the last tone-set in the sequence "
      "(End)," NL "                     instead of outputting forever." NL
      "          -T time   Start at the given clock-time (hh:mm)" NL NL
      ...
      "          -m file   Read audio data from the given file and mix it with "
      "the" NL
      "                      generated binaural beats; may be "
      ...
      "wav/raw format" NL
      "          -M        Read raw audio data from the standard input and mix "
      "it" NL
      "                      with the generated binaural beats (raw only)" NL NL
      "          -R rate   Select rate in Hz that frequency changes are "
      "recalculated" NL
      "                     (for file/pipe output only, default is 10Hz)" NL
      "          -F fms    Fade in/out time in ms (default 60000ms, or 1min)"

      NL
      "          -c spec   Compensate for low-frequency headphone roll-off; "
      "see docs" NL);
  exit(0);
}
...
void
usage() {
  error("SBaGen - Sequenced Binaural Beat Generator, version " VERSION NL
        "Copyright (c) 1999-2011 Jim Peters, http://uazu.net/, all rights " NL
        "  reserved, released under the GNU GPL v2.  See file COPYING." NL NL
        "Usage: sbagen [options] seq-file ..." NL
        "       sbagen [options] -i tone-specs ..." NL
        "       sbagen [options] -p pre-programmed-sequence-specs ..." NL NL
        "For full usage help, type 'sbagen -h'.  For latest version see" NL
        "http://uazu.net/sbagen/ or http://sbagen.sf.net/"
#ifdef EXIT_KEY
        NL NL
        "Windows users please note that this utility is designed to be run as "
        "the" NL
        "associated application for SBG files.  This should have been set up "
        "for you by" NL
        "the installer.  You can run all the SBG files directly from the "
        "desktop by" NL
        "double-clicking on them, and edit them using NotePad from the "
        "right-click menu." NL
        "Alternatively, SBaGen may be run from the MS-DOS prompt (CMD on "
        "WinXP), or from" NL
        "BAT files.  SBaGen is powerful software -- it is worth the effort of "
        "figuring" NL
        "all this out.  See SBAGEN.TXT for the full documentation." NL NL
        "Editing the SBG files gives you access to the full tweakable power of "
        "SBaGen, " NL
        "but if you want a simple GUI interface to the most basic features, "
        "you could " NL "look at a user-contributed tool called SBaGUI:" NL NL
        "  http://sbagen.opensrc.org/wiki.php?page=SBaGUI"
#endif
        NL);
}
...
int fast_mult = 0;                     // 0 to sync to clock (adjusting as necessary), or else sync to output rate, with the multiplier indicated
...
volatile sig_atomic_t should_exit = 0; // Flag set by signal handler to exit gracefully
...
// Use SDL_QueueAudio for both native and Emscripten
SDL_AudioDeviceID aud_device;
...
static inline int
t_per24(int t0, int t1) { // Length of period starting at t0, ending at t1.
  int td = t1 - t0;       // NB for t0==t1 this gives 24 hours, *NOT 0*
  return td > 0 ? td : td + H24;
}

static inline int
t_per0(int t0, int t1) { // Length of period starting at t0, ending at t1.
  int td = t1 - t0;      // NB for t0==t1 this gives 0 hours
  return td >= 0 ? td : td + H24;
}

static inline int
t_mid(int t0, int t1) { // Midpoint of period from t0 to t1
    ...
    while ((opt = *p++)) {
      ...
      switch (opt) {
        ...
        case 'S':
          ...
          break;
        case 'L':
          if (argc-- < 1 || 0 == (val = readTime(*argv, &opt_L)) || 1 == sscanf(*argv++ + val, " %c", &dmy)) {
            error("-L expects hh:mm or hh:mm:ss time");
          }
          break;
        case 'T':
          if (argc-- < 1 || 0 == (val = readTime(*argv, &opt_T)) || 1 == sscanf(*argv++ + val, " %c", &dmy)) {
...
#ifndef MAC_AUDIO
        case 'b':
          if (argc-- < 1 || 1 != sscanf(*argv++, "%d %c", &val, &dmy) || !(val == 8 || val == 16)) {
...
setupOptC(char* spec) {
  ...
  while (1) {
    ...
    if (opt_c >= sizeof(ampadj) / sizeof(ampadj[0])) {
      error("Too many -c option frequencies; maxmimum is %d", sizeof(ampadj) / sizeof(ampadj[0]));
    }
...
sprintVoice(char* p, Voice* vp, Voice* dup) {
  switch (vp->typ) {
    ...
    default:
      ...
      if (dup && vp->typ == dup->typ && vp->carr == dup->carr && vp->res == dup->res && vp->amp == dup->amp) {
        return sprintf(p, "  ::");
      }
...
// State for loop iteration
static int loop_c = 0, loop_cnt = 0;
static int loop_err = 0;
static int loop_fast = 0;
static int loop_vfast = 0;
static int loop_utime = 0;
static int loop_now_lo = 0;
static int loop_err_lo = 0;
static int loop_ms_inc = 0;
...
loop_iteration() {
  ...
  // Check queued audio size - don't generate if we have too much queued
  Uint32 queued = SDL_GetQueuedAudioSize(aud_device);
  // Only generate more if queue is low (less than 0.5 seconds worth)
  if (queued > out_rate * 2 * 2) { // rate * channels * bytes_per_sample
#ifndef __EMSCRIPTEN__
    delay(5); // Sleep briefly to avoid spinning on native
#endif
    return;
  }
  ...
  loop_ms_inc = out_buf_ms + loop_err;
  loop_now_lo += out_buf_lo + loop_err_lo;
  if (loop_now_lo >= 0x10000) {
    loop_ms_inc += loop_now_lo >> 16;
    loop_now_lo &= 0xFFFF;
  }
  now += loop_ms_inc;
  if (now > H24) {
  ...
  if (loop_c >= loop_cnt) {
    ...
    if (loop_fast) {
      ...
      loop_err_lo = diff * 0x10000 / loop_cnt;
      loop_err = loop_err_lo >> 16;
      loop_err_lo &= 0xFFFF;

      if (DEBUG_CHK_UTIME) {
        int prev = loop_utime;
        loop_utime = userTime();
        sprintf(buf, "%d ticks", loop_utime - prev);
      }
      status(buf);
    }
  } else if (loop_vfast && (loop_c & 1)) {
    status(0);
  }
}
...
loop() {
  ...
  loop_cnt = 1 + 1999 / out_buf_ms;
  now = opt_S ? fast_tim0 : calcNow();
  if (opt_T != -1) {
    now = opt_T;
  }
  loop_fast = fast_mult != 0;
  loop_vfast = fast_mult > 20;
  loop_err = loop_fast ? out_buf_ms * (fast_mult - 1) : 0;
  if (opt_L) {
    byte_count = out_bps * (S64)(opt_L * 0.001 * out_rate);
  }
  if (opt_E) {
    byte_count = out_bps * (S64)(t_per0(now, fast_tim1) * 0.001 * out_rate / (loop_fast ? fast_mult : 1));
  }
...
#ifdef __EMSCRIPTEN__
  // Use requestAnimationFrame - call frequently, but only generate when needed
  ...
  if (out_fd == -9999) {
    SDL_PauseAudioDevice(aud_device, 1);
    SDL_CloseAudioDevice(aud_device);
    SDL_Quit();
  }
...
writeOut(char* buf, int siz) {
  ...
  if (out_fd == -9999) {
    // Use SDL_QueueAudio for both native and Emscripten
    SDL_QueueAudio(aud_device, buf, siz);
    return;
  }
...
setup_device(void) {
  ...
  if (opt_O || opt_o) {
    ...
    if (!opt_Q && !opt_W) { // Informational message for opt_W is written later
      warn(
          "Outputting %d-bit raw audio data at %d Hz with %d-sample blocks, %d "
          "ms per block",
  ...
  // SDL2 audio output
  {
    ...
    want.channels = 2;
    want.samples = 4096;
    want.callback = NULL; // Use SDL_QueueAudio - no callback
    want.userdata = NULL;

    aud_device = SDL_OpenAudioDevice(NULL, 0, &want, &have, 0);
    if (aud_device == 0) {
      error("SDL_OpenAudioDevice failed: %s", SDL_GetError());
    }

    if (have.format != AUDIO_S16SYS) {
      error("SDL audio format mismatch");
    }

    SDL_PauseAudioDevice(aud_device, 0); // Start playback

    out_rate = have.freq;

    if (have.channels != 2) {
      error("SBaGen requires stereo output -- SDL got %d channels", have.channels);
    }

    if (!opt_Q) {
      warn("Outputting 16-bit audio at %d Hz via SDL2,\n"
           "  using %d-sample fragments, %d ms per fragment",
           out_rate, out_blen / 2, out_buf_ms);
    }
  }
}
...
writeWAV() {
  ...
  if (byte_count + 36 != (int)(byte_count + 36)) {
    ...
    warn("WARNING: Selected length is too long for the WAV format; truncating to %dh%02dm%02ds", tmp / 3600, tmp / 60 % 60, tmp % 60);
  }
  ...
  if (!opt_Q) {
    warn("Outputting %d-bit WAV data at %d Hz, file size %d bytes", out_mode ? 16 : 8, out_rate, byte_count + 44);
  }
}
...
correctPeriods() {
        ...
        for (a = 0; a < N_CH; a++) {
          ...
          if ((fo == 0 || fi == 0) ||  // Fade in/out to silence
              ...
              ((fo == 1 || fi == 1) && // Fade thru, but different pitches
               (vp->typ == 1 || vp->typ < 0) && (vp->carr != vq->carr || vp->res != vq->res))) {
  ...
  // Clear out zero length sections, and duplicate sections
  {
    ...
    while (per != per->nxt) {
      pp = per;
      do {
        if (voicesEq(pp->v0, pp->v1) && voicesEq(pp->v0, pp->nxt->v0) && voicesEq(pp->v0, pp->nxt->v1)) {
          pp->nxt->tim = pp->tim;
        }
      ...
      do {
        if (t_per0(pp->tim, pp->nxt->tim) >= H12) {
          warn("  %02d:%02d:%02d -> %02d:%02d:%02d",
               pp->tim % 86400000 / 3600000, pp->tim % 3600000 / 60000,
               pp->tim % 60000 / 1000, pp->nxt->tim % 86400000 / 3600000,
               pp->nxt->tim % 3600000 / 60000, pp->nxt->tim % 60000 / 1000);
        }
        pp = pp->nxt;
      } while (pp != per);
...
readNameDef() {
  ...
  *q = 0;
  ...
  // Waveform definition ?
  if (0 == memcmp(p, "wave", 4) && isdigit(p[4]) && isdigit(p[5]) && !p[6]) {
    ...
    while ((p = getWord())) {
      ...
      if (1 != sscanf(p, "%lf %c", &dd, &dmy)) {
        error(
            "Expecting floating-point numbers on this waveform "
            "definition line, line %d:\n  %s",
            in_lin, lin_copy);
      }
      if (dp >= dp1) {
        error("Too many samples on line (maximum %d), line %d:\n  %s", dp1 - dp0, in_lin, lin_copy);
      }
      *dp++ = dd;
      if (dmax < dmin) {
        dmin = dmax = dd;
  ...
  if (*lin == '{') {
    BlockDef *bd, **prvp;
    if (!(p = getWord()) || 0 != strcmp(p, "{") || 0 != (p = getWord())) {
      badSeq();
    }
...
readTimeLine() {
  ...
  if (nd->blk) {
    ...
    while (bd) {
      ...
      strcpy(lin_copy, lin);
      readTimeLine(); // This may recurse, and that's why we're StrDuping the
                      // string
      bd = bd->nxt;
    }
...
readTime(char* p, int* timp) { // Rets chars consumed, or 0 error
  ...
  if (3 > sscanf(p, "%2d:%2d:%2d%n", &hh, &mm, &ss, &nn)) {
    ss = 0;
    if (2 > sscanf(p, "%2d:%2d%n", &hh, &mm, &nn)) {
      return 0;
    }
  }
...
void
sinc_interpolate(double* dp, int np, int* arr) {
...
bad_drop() {
  error(
      "Bad arguments: expecting -p drop [<time-spec>] <drop-spec> "
      "[<tone-specs...>]" NL
...
create_drop(int ac, char** av) {
  ...
  double beat_target;
  double beat[40];
  static double beat_targets[] = {4.4, 3.7, 3.1, 2.5, 2.0, 1.5, 1.2, 0.9, 0.7, 0.5, 0.4, 0.3};
  char extra[256];
...
#undef BAD
  ...
  if (slide) {
    ...
    warn(" Beat frequency slides from %gHz to %gHz over %d minutes", beat[0], beat[n_step - 1], len0 / 60);
  } else {
    warn(" Carrier steps from %gHz to %gHz over %d minutes", c0, c2, len / 60);
    warn(" Beat frequency steps from %gHz to %gHz over %d minutes:", beat[0], beat[n_step - 1], len0 / 60);
    fprintf(stderr, "   ");
    for (a = 0; a < n_step; a++) {
  ...
  if (slide) {
    ...
    for (a = 0; a < n_step; a++) {
      ...
      formatNameDef("ts%02d: %g+%g/%g %s", a, c0 + (c2 - c0) * tim * 1.0 / len, beat[a], amp, extra);
      formatTimeLine(tim, "== ts%02d ->", a);
    }

    if (islong) {
      formatNameDef("tsend: %g+%g/%g %s", c2, beat[n_step - 1], amp, extra);
    ...
    for (a = 0; a < lim; a++) {
      ...
      formatNameDef("ts%02d: %g+%g/%g %s", a, c0 + (c2 - c0) * tim1 / len, beat[(a >= n_step) ? n_step - 1 : a], amp, extra);
  ...
  // Wake-up and ending
  if (wakeup) {
    formatNameDef("tswake: %g+%g/%g %s", c0, beat[0], amp, extra);
...
bad_slide() {
  error(
      "Bad arguments: expecting -p slide [<time-spec>] <slide-spec> "
      "[<tone-specs...>]" NL
...
#define BAD bad_slide()
  ...
  if (3 != sscanf(av[0], "%lf%lf/%lf %c", &c0, &beat, &amp, &dmy)) {
    BAD;
  }
  c1 = beat / 2;
  ac--;
...
#undef BAD
  ...
  *p = 0;
  ...
  // Summary
  warn("SLIDE summary:");
  warn(" Sliding carrier from %gHz to %gHz over %g minutes", c0, c1, len / 60.0);
